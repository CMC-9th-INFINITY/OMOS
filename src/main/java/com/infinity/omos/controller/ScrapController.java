...
public class ScrapController {
    ...
    public ResponseEntity<StateDto> saveScrap(@PathVariable("postsId") Long postId, @PathVariable("userId") Long userId) {
        return ResponseEntity.ok(scrapService.save(postId, userId));
    }

    @ApiOperation(value = "스크랩 취소", notes = "1.해당 레코드는 존재하지 않는 레코드입니다\n2.해당 유저는 존재하지 않는 유저입니다 의 오류가 존재합니다")
    @DeleteMapping("/delete/{postsId}/{userId}")
    public ResponseEntity<StateDto> deleteScrap(@PathVariable("postsId") Long postId, @PathVariable("userId") Long userId) {
